...
AGV 티칭 포인트(좌표/이름/설비종류/상태)를 표와 **실측 2D 맵**으로 관리합니다.

**기본 동작**
- **샘플** : 예제 프로젝트 불러오기
- **포인트 추가 / 삭제** : 표에서 행 편집(이름·x·y·θ·설비ID 직접 수정)
- **검증(Validate)** : 좌표 중복·이름 중복·LOAD/UNLOAD 누락·**알람 포인트** 등 점검 후 로그 표시
- **열기… / 저장…** : 프로젝트 JSON 입출력 · **CSV 내보내기…** : 표를 CSV로 저장

**배경 도면 (CAD)** — 하단 *배경 도면(CAD)* 탭
- **이미지 불러오기…** 로 도면/평면도(PNG·JPG·BMP)를 맵 배경으로 깔고 그 위에서 티칭합니다.
- **축척(mm/픽셀)** 으로 실제 치수에 맞추고, **투명도** 슬라이더로 가독성을 조절합니다.
- (CAD를 PNG/JPG로 내보내 사용하세요. 벡터 DXF 직접 로딩은 로드맵 항목입니다.)

**설비 종류 커스텀** — 하단 *설비 종류* 탭
- 설비 종류마다 **색상**과 **마커 모양**(원/사각형/삼각형/마름모/별/＋/✕)을 자유롭게 지정합니다.
- **종류 추가**로 새 설비 타입을 만들고, 표의 *설비 종류* 칸에서 포인트에 적용합니다.
- 맵에서 **모양 = 설비 종류**, **테두리색 = 종류색** 으로 표시됩니다.

**티칭 상태** — 표의 *상태* 칸 (또는 상단 대시보드)
- **진행중(노랑) / 완료(초록) / 알람(빨강)** 을 포인트별로 저장합니다.
  *알람* = 티칭은 했으나 정상 동작하지 않아 재작업이 필요한 상태.
- 맵 마커의 **채움색 = 상태**, 상단 칩이 상태별 개수를 요약합니다.
